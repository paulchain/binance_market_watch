...
try{
	...
	// Get current price of this symbol

	$price = $api->price($market);
	...
	for($i=0; $i<$total; $i++){
		...
		if($trades[$i]['maker'] == 'true'){
			$maker += $trades[$i]['quantity'];
			// echo $trades[$i]['quantity'] . "<br />";
		}else{
			$taker += $trades[$i]['quantity'];
			// echo $trades[$i]['quantity'];
		}
	}

	$buySellRate = round($taker/$maker, 2);
	...
	$symbol = str_replace("BTC", "", $market);

	echo 'Latest price: ', $price, ' BTC<br /><br />';

	echo 'Sell: <font color="red">', $maker ,' ', $symbol, '</font> - Buy: <font color="green">', $taker ,' ', $symbol ,'</font><br /><br />';

	echo '<font color="', $cssColor, '"><strong>Buy/Sell Rate: </strong>', $buySellRate ,'</font><br /><br />';
	?>
	Please install this plugin to let page reload automatically: <a href="https://chrome.google.com/webstore/detail/auto-refresh/ifooldnmmcmlbdennkpdnlnbgbmfalko" target="_blank">Auto Refresh Chrome Plugin</a>
	<?php
}
catch(\Error $e){
	echo $e->getMessage();
